...
These targeting modes assume that the token is both properly aligned to the grid, as well as having whole numbers for their height and width. If the token is not aligned to the grid, the points that are generated will not be properly aligned with the grid either. If the token has a height or width that isn't a whole number, a ceiling operation is performed to make it a whole number before points are generated. These are both limitations of the current implementation, and if the conditions are not met then the targeting methods may not perform as desired.
